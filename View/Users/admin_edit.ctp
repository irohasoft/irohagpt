...
<?php $this->Html->scriptStart(['inline' => false]); ?>
	$(function (e) {
		...
		//$('#TemplateTemplate').select2({placeholder: "<?= __('使用するテンプレートを選択して下さい。(複数選択可)')?>", closeOnSelect: <?= (Configure::read('close_on_select') ? 'true' : 'false'); ?>,});
		// パスワードの自動復元を防止
...
<?= $this->Html->link(__('<< 戻る'), ['action' => 'index'])?>
	<div class="panel panel-default">
		...
		<?php
			...
			//echo $this->Form->input('Template',				['label' => __('テンプレート')]);
			...
			// 編集の場合のみ、チャット履歴削除ボタンを表示
			if($this->isEditPage())
			{
				echo $this->Form->postLink(__('チャット履歴を削除'),
					['action' => 'clear', $this->request->data['User']['id']],
					['class' => 'btn btn-default pull-right btn-clear'],
					__('チャット履歴を削除してもよろしいですか？', $this->request->data['User']['name']));
			}
		?>
		</div>
	</div>
